function [ f_exp, g_exp, B_exp] = computeExponentialConstr_fgB(x, functionParams , params)
...
for i = 1:n_well_type
    for j = 1:T
        ...
        factor = lambda(start+cur_index) - mu*f_exp(cur_index);
        q_pot_i = q_pot(cur_index);
        d_i = d(cur_index);
        t_cur = t_ij(cur_index);
        g_exp(start+cur_index,cur_index) = deltaT;
        if j == 1
            g_exp(ind,cur_index) = -q_pot_i*exp(-d_i*t_cur);
            B_exp(ind,ind) = B_exp(ind,ind) + factor*q_pot_i*d_i*exp(-d_i*t_cur);
        else
            t_prev = t_ij(cur_index-1);
            g_exp(ind-1,cur_index) = q_pot_i*exp(-d_i*t_prev);
            g_exp(ind,cur_index) = -q_pot_i*exp(-d_i*t_cur);
            B_exp(ind-1,ind-1) = B_exp(ind-1,ind-1)-factor*q_pot_i*d_i*exp(-d_i*t_prev);
            B_exp(ind,ind) = B_exp(ind,ind)+factor*q_pot_i*d_i*exp(-d_i*t_cur);
        end
    end
end
