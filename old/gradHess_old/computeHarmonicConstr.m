...
B_har = zeros(length(x),length(x), n_well_type*T);
...
for i = 1:n_well_type
    for j = 1:T
        ...
        if j == 1
            ...
            B_har(N*T + start + cur_index,N*T +start + cur_index,cur_index) = q_pot_i*d_i/(1+d_i*t_cur)^2;
        else
            t_prev = t_ij(cur_index-1);
            g_har(N*T + start + cur_index-1,cur_index) = q_pot_i/(1+d_i*t_prev);
            g_har(N*T + start + cur_index,cur_index) = -q_pot_i/(1+d_i*t_cur);
            B_har(N*T + start + cur_index-1,N*T + start +cur_index-1,cur_index) = -q_pot_i*d_i/(1+d_i*t_prev)^2;
            B_har(N*T + start + cur_index,N*T + start + cur_index,cur_index) = q_pot_i*d_i/(1+d_i*t_cur)^2;
        end
    end
end
