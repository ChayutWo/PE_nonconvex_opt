function [ f_exp, g_exp, B_exp] = computeExponentialConstr( x, params )
...
B_exp = zeros(length(x),length(x), n_well_type*T);
...
for i = 1:n_well_type
    for j = 1:T
        ...
        if j == 1
            ...
            B_exp(N*T + start + cur_index,N*T +start + cur_index,cur_index) = q_pot_i*d_i*exp(-d_i*t_cur);
        else
            t_prev = t_ij(cur_index-1);
            g_exp(N*T + start + cur_index-1,cur_index) = q_pot_i*exp(-d_i*t_prev);
            g_exp(N*T + start + cur_index,cur_index) = -q_pot_i*exp(-d_i*t_cur);
            B_exp(N*T + start + cur_index-1,N*T + start +cur_index-1,cur_index) = -q_pot_i*d_i*exp(-d_i*t_prev);
            B_exp(N*T + start + cur_index,N*T +start + cur_index,cur_index) = q_pot_i*d_i*exp(-d_i*t_cur);
        end
    end
end
